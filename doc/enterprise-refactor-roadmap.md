...
- **Pipeline-run LLM usage pass completed on June 12, 2026:** Agent-recorded `ai_usage_events` and `pipeline_run_usage_summary` now surface through Pipeline Runs cards, run detail, step detail, task detail, and run-scoped Monitoring LLM Usage links. Monitoring LLM Usage includes `runId` filtering plus pipeline, step, task, feature, profile, model, subject, exact/estimated, trend, and top-run breakdowns. Monitoring route transport was moved into `src/features/monitoring/api.ts` to keep route-local `apiClient.fetch` below the enforced baseline.
...
Latest pipeline-run LLM usage verification on June 12, 2026 used host Go and Dockerized Node 22 because the host shell did not expose `npm`. `go test ./...` passed. `docker compose run --rm ui-test sh -c "npm ci && npm run lint && npm run check:ui-boundaries && npm run test && npm run build"` passed: lint passed; boundary check reported 0 violations, 10/11 route-local `apiClient.fetch` calls, 2 report-only large route files, 1 report-only large feature-shell file, and 108 report-only browser `window.*` usages; 105 unit tests passed; 133 component tests passed with 23.45% statements, 21.25% branches, 24.90% functions, and 24.39% lines against enforced floors of 21%, 18%, 22%, and 22%; the production build completed successfully. `docker compose run --rm ui-e2e` passed all 9 mocked Playwright workflows, and `docker compose run --rm ui-e2e sh -c "npm ci && npm run test:e2e:live"` skipped both protected live tests because credentials and the mutation fixture are not present locally.

## Change Log

- **June 12, 2026:** Added pipeline-run LLM token summaries and run-scoped Monitoring drilldowns, including step/task/profile token aggregation, route-owned orchestration with feature-owned Monitoring transport, API/docs updates, and focused backend/UI coverage.
