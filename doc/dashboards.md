...
The GitOps sample includes an immediately runnable dashboard pipeline at
`examples/gitops-quickstart/team-repo/pipelines/platform/service-health-dashboard.yaml`.
It uses only an `alpine:3.20` shell script and structured
`dashboard_evidence=<json>`, so it can be launched directly or through the bound
`platform/service-health` dashboard source without secrets, approvals,
variables, or external MCP profiles.
