...
- **Workspace sharing controls**: Shares a sanitised directory listing with the LLM when `llm_content_preload` is enabled, bounded by `llm_content_include` and `llm_content_ignore`. Those two patterns build the workspace index, so they also bound what the workspace retrieval tools can read.
