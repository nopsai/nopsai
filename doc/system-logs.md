...
enables `kubernetes,docker` providers by default with a release-scoped selector,
so Docker runner logs can resolve when `systemLogs.dockerHost` points at a
restricted Docker API endpoint reachable from the API pod.
...
ingestion. A Docker-hosted control plane needs a kubeconfig or in-cluster-style
credentials for Kubernetes runners, and a Kubernetes-hosted control plane needs
an explicit Docker API/socket-proxy endpoint for Docker runners. Runner sources
are also checked against dispatcher registration, so
...
Compose declares the socket proxy, allow-lists the persistent platform containers, and sets `SYSTEM_LOGS_PROVIDER=docker,kubernetes` with `SYSTEM_LOGS_DOCKER_HOST=tcp://docker-socket-proxy:2375`. Other Docker deployments can configure the feature declaratively in the mounted NopsAI YAML:
...
Kubernetes deployments select Kubernetes first and Docker when an endpoint is supplied:

```yaml
system_logs:
  enabled: true
  provider: kubernetes,docker
  docker_host: tcp://docker-socket-proxy.example.internal:2375
  kubernetes:
    namespace: nopsai
...
`SYSTEM_LOGS_PROVIDER=kubernetes,docker`, `SYSTEM_LOGS_KUBERNETES_NAMESPACE`,
