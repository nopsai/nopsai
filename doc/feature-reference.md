...
- Prometheus-friendly `GET /metrics` with DB-backed pipeline run, duration,
  ...
  final output, external trigger, identity provider capability, authorization
  grant ownership, Knowledge Context connection health and sync status/cache
  age, LLM usage, runner capacity, approval wait, audit, and notification
  delivery metrics
...
container or pod restart markers. Server-side best-effort redaction occurs
before bounded in-memory buffering; Docker access is isolated behind a
read-only socket proxy, and Kubernetes access uses read-only pod/log RBAC. See
[system-logs.md](./system-logs.md).
