(function (global) {
  ...
  function showLogsModal() {
    ...
    // Fetch historical logs, new ones will come via WebSocket or Polling
    state.lastLogId = 0; // Reset cursor
  ...
  function startLogPolling(runId) {
    stopLogPolling();
    const poll = async () => {
      if (!DOM.logsModal || DOM.logsModal.classList.contains('hidden')) return;

      await fetchAndRenderLogs(runId);

      const isHidden = document.visibilityState === 'hidden';
      const interval = isHidden ? 30000 : 2000;
      state.logPollingTimer = setTimeout(poll, interval);
    };
    // Initial delay before first poll
    state.logPollingTimer = setTimeout(poll, 2000);
  }

  function stopLogPolling() {
    if (state.logPollingTimer) {
      clearTimeout(state.logPollingTimer);
      state.logPollingTimer = null;
    }
  }
