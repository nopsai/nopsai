...
export function DocumentPreview({ document }: { document: DocumentSpec }) {
  return (
    <article className="mx-auto max-w-4xl bg-white p-5 text-sm leading-6 text-slate-800 dark:bg-slate-950 dark:text-[var(--text-primary)]">
      <header className="mb-6 border-b-2 border-blue-600 pb-4">
        <h4 className="text-2xl font-bold text-slate-950 dark:text-[var(--text-primary)]">{document.title}</h4>
      ...
      <div className="space-y-6">
        {document.sections.map((section, sectionIndex) => (
          <section key={`${section.title}-${sectionIndex}`}>
            <h5 className="mb-3 text-lg font-bold text-slate-900 dark:text-[var(--text-primary)]">{section.title}</h5>
    ...
    return (
      <div className="overflow-x-auto border border-slate-300 dark:border-slate-700">
        <table className="min-w-full border-collapse text-left text-xs">
          <thead className="bg-blue-50 text-slate-900 dark:bg-slate-800 dark:text-[var(--text-primary)]">
