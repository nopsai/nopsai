...
export function useAccessPanelController({
  ...
  const [policyEditor, setPolicyEditor] = useState<PolicyEditorState | null>(null);
  const [confirmDialog, setConfirmDialog] = useState<{ message: string; onConfirm: () => Promise<void> | void } | null>(null);
  const [confirming, setConfirming] = useState(false);
  const [userAccessEditor, setUserAccessEditor] = useState<UserAccessEditorState | null>(null);
  const [serviceAccountEditor, setServiceAccountEditor] = useState<ServiceAccountEditorState | null>(null);
