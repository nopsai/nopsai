...
export function normalizeProfilePayload(payload: unknown): string[] {
  ...
  return normalizeAutocompleteList(profiles);
}
...
export async function fetchEditorAutocompleteMetadata(
  ...
  const [secretsResp, variablesResp, stepsResp, secretScopesResp, variableScopesResp, llmProfilesResp, mcpProfilesResp] = await Promise.all([
