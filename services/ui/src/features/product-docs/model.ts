...
const pipelineTopLevelRows: WikiConfigRow[] = [
  ...
  },
  {
    ...
    area: 'Pipeline YAML',
    description: 'AI governance enforcement level. advisory warns, guarded blocks clear violations and conflicts, strict allows only clear approval, and exception_based requires approved exceptions for conflicts.',
    example: 'strict',
    type: 'enum',
    required: false,
    defaultValue: 'strict',
    scope: 'pipeline, step, task',
    constraints: ['Supported values are advisory, guarded, strict, and exception_based. Step and task values override the pipeline value for that narrower decision scope.'],
  },
  {
    key: 'runtime_pool',
    area: 'Pipeline YAML',
    ...
    articles: [
      ...
      },
      {
        ...
        details: [
          ...
          'During cold starts, setup preflight keeps /healthz unready while retrying a database that is still starting; normal API startup resumes automatically once Postgres is reachable.',
    ...
    articles: [
      ...
      },
      {
        ...
        details: [
          ...
          'Kubernetes installs generated by the CLI include a Secret manifest for new environments; apply it directly for bootstrap or encrypt/seal it with External Secrets, Sealed Secrets, SOPS, or a cluster secret manager before GitOps.',
        ...
        examples: [
          ...
          {
            title: 'Generate Kubernetes install files',
            language: 'bash',
            code:
      ...
      },
      {
        ...
        summary:
          'The Helm chart deploys PostgreSQL, API, AAA, dispatcher, git-bot, UI, Gotenberg, and a Kubernetes runner. CLI-generated installs keep values non-secret and write a separate bootstrap Secret manifest.',
        keyFacts: [
          '`nopsai install kubernetes` generates values.yaml, nopsai-secrets.yaml, installation.md, and install lock metadata for new environments.',
        ...
        details: [
          'Review installation.md, apply the generated Secret manifest directly for bootstrap, or encrypt/seal it with External Secrets, Sealed Secrets, SOPS, or a platform secret manager before committing it to GitOps.',
        ...
        examples: [
          {
            title: 'Generate and deploy Kubernetes install files',
            language: 'bash',
            code:
              'nopsai install kubernetes \\\n  --version <version> \\\n  --output-dir ./nopsai-prod \\\n  --values-file values.yaml \\\n  --secret-file nopsai-secrets.yaml\n\ncd ./nopsai-prod\nkubectl apply -f nopsai-secrets.yaml\nnopsai install kubernetes --output-dir . --values-file values.yaml --deploy --wait',
          },
      ...
      },
      {
        ...
        relatedDocs: ['doc/kubernetes-runner.md', 'doc/runtime-flows.md', 'doc/runner-registry-auth.md'],
      ...
      {
        id: 'private-registry-runner-auth',
        title: 'Private Registry Runner Auth',
        level: 'Admin',
        docType: 'reference',
        audience: 'Administrators and platform operators',
        audiences: ['administrator', 'operator', 'security'],
        summary:
          'Runner private registry access uses selected docker_config_json credentials for one-time bootstrap, local Docker runner auth config, and Kubernetes imagePullSecrets without exposing registry values in install commands.',
        keyFacts: [
          '`docker_config_json` credentials store encrypted Docker config JSON and expose only non-secret registry host metadata.',
          'Runner install endpoints accept `registry_credential_ref` and `registry_credential_refs` query parameters for selected credential references.',
        ...
        configRows: [
          {
            ...
            area: 'Credentials',
            description: 'Credential kind for encrypted Docker config JSON used by runner registry auth.',
            example: '{"auths":{"ghcr.io":{"auth":"..."}}}',
            security: 'Value is encrypted and write-only; metadata exposes only registry hostnames.',
          },
          {
            key: 'registry_credential_ref',
            area: 'Runner install API',
            description: 'Repeated query parameter selecting one registry credential for a one-time runner install command.',
            example: 'credential://system/registry/ghcr',
            permission: 'dispatcher runner management plus credential read access',
          },
          {
            key: 'registry_credential_refs',
            area: 'Runner install API',
            description: 'Comma-separated query parameter alternative for selecting multiple registry credentials.',
            example: 'credential://system/registry/ghcr,credential://system/registry/harbor',
            permission: 'dispatcher runner management plus credential read access',
          },
          {
            key: 'runner_registry_credentials',
            area: 'GitOps',
            description: 'Map from runner ID to selected docker_config_json credential references.',
            example: 'runner-prod-1: [credential://system/registry/ghcr]',
            path: 'setting/system/runner.yaml.runner_registry_credentials',
            security: 'Stores references only, never Docker config values.',
          },
          {
            key: 'global.imagePullSecrets',
            area: 'Helm',
            description: 'Infrastructure-managed chart pull secrets for NopsAI release images.',
            example: '- name: nopsai-registry',
            security: 'Separate from NopsAI runner registry assignments.',
          },
        ],
        examples: [
          {
            title: 'Assign registry credentials through GitOps',
            language: 'yaml',
            code:
          ...
          {
            title: 'Generate a Docker runner install with registry access',
            language: 'bash',
            code:
              'curl -H "Authorization: Bearer $NOPSAI_TOKEN" \\\n  "https://nopsai.example.com/v1/system/dispatcher/runner-bootstrap-command?runner_id=runner-prod-1&runner_scopes=prod&registry_credential_ref=credential://system/registry/production-ghcr"',
            permission: 'System dispatcher runner management and credential read access',
          },
        ],
        relatedDocs: ['doc/runner-registry-auth.md', 'doc/credential-management.md', 'doc/kubernetes-runner.md', 'doc/api.md'],
        runbooks: ['Bootstrap private Docker runner image', 'Attach Kubernetes runner imagePullSecret', 'Rotate runner registry credential'],
        caveats: [
          'The raw Kubernetes runner manifest endpoint is intentionally secret-free; create imagePullSecrets through infrastructure when committing raw manifests to a cluster GitOps repository.',
        ],
      },
    ],
  },
    ...
    articles: [
      {
        ...
        keyFacts: [
          ...
          'governance_level controls how NopsAI interprets AI policy reviews: advisory warns, guarded blocks clear violations and conflicts, strict blocks anything except a clear allow, and exception_based requires an approved exception for conflicts.',
        ...
        details: [
          ...
          'Profile directives are intentionally separate. agent_profile changes the persona and instructions; llm_profile selects the provider and model client; mcp_profiles selects approved external tools; knowledge_context supplies governed documents; governance_level tells NopsAI how strongly to enforce policy judgments.',
      ...
      },
      {
        ...
        details: [
          ...
          'Fullscreen expanded toolbox insertion is UI-only authoring help. Saves still use the existing YAML mutation path, AAA checks, and GitOps database-override warnings.',
        ],
      ...
      },
      {
        ...
        keyFacts: [
          ...
          'The Trigger editor fullscreen expanded toolbox lists provider, team, event-rule, branch/path, pipeline, and scope fields and can insert valid trigger rule samples at the current cursor.',
        ],
        details: [
    ...
    articles: [
      {
        ...
        configRows: [
          ...
          {
            key: 'config-repositories/teams/<team>/defaults.yaml',
            area: 'Team GitOps',
            description: 'Team LLM, Agent, and knowledge-kind defaults.',
            example: 'config-repositories/teams/platform/ml/defaults.yaml',
          },
        ],
        examples: [],
        relatedDocs: ['doc/agent-profiles.md', 'doc/llm-model-selection.md'],
        runbooks: ['Review a new Agent Profile', 'Limit profile use by scope', 'Audit team profile defaults'],
      ...
      },
      {
        ...
        details: [
          ...
          'Provider prompt caches and provider conversation state are transport optimizations. NopsAI still owns the logical session, scoped context, governance contract, and cache identity used for audit and invalidation.',
      ...
      },
      {
        ...
        keyFacts: [
          ...
          'Governance levels define enforcement: advisory logs or warns, guarded blocks clear violations and conflicts, strict requires a clear allow, and exception_based needs an approved exception for conflicts.',
        ...
        details: [
          ...
          'History records what happened, but the current effective Knowledge Context and governance_level remain authoritative for new decisions.',
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        title: 'Analysis Reviewers',
        level: 'Operate',
        audience: 'Operators, automation authors, team owners, and security reviewers',
        summary:
          'The UI provides three read-only reviewer buttons with explained scoring and optional LLM-profile-backed AI evaluation: Analyse Resources on Teams, Analyse Pipeline on pipeline detail, and Analyse Run on run detail.',
        keyFacts: [
          ...
          'AI Evaluation asks for structured JSON and the modal renders separate Problem, Why This Score, scored finding impacts, Suggested Fixes, and More Evidence Needed sections instead of raw model output.',
          'Structured AI reviews are cached in browser-local history by subject type, subject ID, and snapshot revision, so reopening the same snapshot can reuse the reviewed score without another LLM call. If the page snapshot changed, the latest cached review is still shown as previous-snapshot context until a fresh AI review is generated.',
          'The reviewer does not expose credential values and does not mutate pipelines, resources, permissions, or runs.',
        ],
        details: [
          ...
          'AI Evaluation uses existing Assistant feature flags, LLM profile scope, credential reference resolution, and per-request usage reporting. If the Assistant is disabled or no usable LLM profile is available, deterministic analysis still works and the modal links operators to LLM Profiles.',
        ],
        configRows: [
          {
            key: 'Analyse Resources',
            area: 'Teams UI',
            description: 'Team/application resource reviewer for organization, security, efficiency, reuse, and unused-resource signals.',
            example: 'Teams > Resources > Analyse Resources',
            permission: 'Same visible team/resource access already required by Teams',
            security: 'Uses metadata only, redacts credential-like values, and sends only redacted findings if AI Evaluation is requested.',
          },
          {
            key: 'Analyse Pipeline',
            area: 'Pipelines UI',
            description: 'Pipeline detail reviewer with complete, category-specific, run-history, and pre-execution modes.',
            example: 'Pipelines > pipeline detail > Analyse Pipeline',
            permission: 'Same pipeline read access already required by the pipeline detail page',
            security: 'Does not execute the pipeline. AI Evaluation uses the analysis evaluation API and redacted reviewer report, not direct browser-to-provider calls or hosted MCP planner calls.',
          },
          {
            key: 'Analyse Run',
            area: 'Pipeline Runs UI',
            description: 'Run detail reviewer for first-failure detection, domain classification, last-success comparison, and success-run degradation.',
            example: 'Pipeline Runs > run detail > Analyse Run',
            permission: 'Same pipeline_run.read access already required by run detail',
            security: 'Read-only; suggested actions open logs or existing resource views. AI Evaluation is automatic for runs when Assistant debugging is enabled.',
          },
        ],
        examples: [
          {
            title: 'Read a failed-run diagnosis',
            language: 'text',
            code:
              'Open a failed run, select Analyse Run, review the Primary diagnosis, inspect First failed execution point, then open logs for that step before changing the pipeline or rerunning.',
            complete: true,
            testedIn: DEFAULT_VERIFIED_DATE,
            permission: 'pipeline_run.read for the selected run',
          },
        ],
        relatedDocs: ['doc/feature-reference.md', 'doc/mcp-feature-coverage.md'],
        sourceLinks: [
          {
            title: 'Shared reviewer model and safeguards',
            repositoryPath: 'services/ui/src/features/analysis/model.ts',
            purpose: 'Owns analysis finding types, deterministic score provenance, resource checks, pipeline checks, and run diagnosis.',
          },
          {
            title: 'AI prompt shaping',
            repositoryPath: 'services/ui/src/features/analysis/ai.ts',
            purpose: 'Builds subject-specific prompts, redacted score snapshots, page context, and structured AI Evaluation parsing.',
          },
          {
            title: 'AI transport',
            repositoryPath: 'services/ui/src/features/analysis/api.ts',
            purpose: 'Selects a usable LLM profile and calls /v1/analysis/evaluate for reviewer AI Evaluation without creating Assistant conversations.',
          },
          {
            title: 'AI-reviewed score overlay',
            repositoryPath: 'services/ui/src/features/analysis/reviewedScore.ts',
            purpose: 'Converts structured AI scored findings into active health/category scores while retaining the deterministic score as baseline provenance.',
          },
          {
            title: 'AI review cache',
            repositoryPath: 'services/ui/src/features/analysis/evaluationCache.ts',
            purpose: 'Stores browser-local structured AI review history keyed by subject type, subject ID, and exact snapshot revision.',
          },
          {
            title: 'AI orchestration hook',
            repositoryPath: 'services/ui/src/features/analysis/useAnalysisAiEvaluation.ts',
            purpose: 'Owns loading, retry, stale snapshot protection, cached-review hydration, and automatic run-analysis AI evaluation.',
          },
          {
            title: 'Run AI evidence context',
            repositoryPath: 'services/ui/src/features/pipeline-runs/runAnalysisEvidence.ts',
            purpose: 'Builds bounded redacted failed-step configuration, YAML, and log context before run AI Evaluation.',
          },
          {
            title: 'Pipeline AI evidence context',
            repositoryPath: 'services/ui/src/features/pipelines/pipelineAnalysisEvidence.ts',
            purpose: 'Builds bounded redacted YAML, validation, graph, trigger, dependency, and recent-run context before pipeline AI Evaluation.',
          },
          {
            title: 'Team/resource AI evidence context',
            repositoryPath: 'services/ui/src/features/teams/teamAnalysisEvidence.ts',
          ...
          },
          {
            ...
            repositoryPath: 'services/ui/src/features/teams/TeamsWorkspace.tsx',
            purpose: 'Places Analyse Resources and per-resource Analyse actions in the Team Resources browser.',
          },
          {
          ...
          },
          {
            ...
            repositoryPath: 'services/ui/src/features/pipeline-runs/RunDetailPanel.tsx',
            purpose: 'Places Analyse Run beside Logs and Usage and connects safe follow-up actions.',
          },
        ],
        runbooks: ['Review duplicate deployment pipelines', 'Diagnose failed run first failure', 'Run pre-execution pipeline analysis'],
        caveats: [
          'Generated YAML patches, issue drafts, and direct server-side analysis records are not implemented in this UI layer.',
          'AI Evaluation is explanatory and can be limited by Assistant configuration, LLM profile scope, credential references, model access, and available visible evidence.',
          'Run log excerpts are available through the existing Logs modal; the reviewer does not persist separate analysis records.',
        ],
      },
      {
        id: 'notifications',
        ...
        examples: [
          {
            ...
            language: 'json',
            code:
              '{\n  "productVersion": "<version>",\n  "apiVersion": "v1",\n  "cliCompatibility": "<cli-version-range>",\n  "runnerCompatibility": "<runner-version-range>",\n  "capabilities": [\n    "api.v1",\n    "cli.api-catalog.v1",\n    "config-sync.v1",\n    "dashboard-refresh.v1",\n    "dashboards.v1",\n    "mcp.v1",\n    "monitoring.v1",\n    "platform.helm",\n    "runner.docker",\n    "runner.kubernetes"\n  ]\n}',
            complete: true,
            testedIn: DEFAULT_VERIFIED_DATE,
          },
        ],
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        summary:
          'The released `nopsai` CLI manages contexts, credentials, generic API access, platform diagnostics, completion files, GitOps deployment, generated Kubernetes installation files, and release verification.',
        keyFacts: [
          ...
          'When Postgres is still starting, API setup preflight remains unready while the server retries the database and should recover automatically once it is reachable.',
          ...
          'Generated Kubernetes installs include values.yaml, nopsai-secrets.yaml, installation.md, and install lock metadata; installation.md covers requirements, pull secrets, ServiceAccounts, applying secrets, values review, Helm deploy, verification, and generated-secret handling.',
    ...
    articles: [
      ...
      },
      {
        ...
        keyFacts: [
          ...
          'A temporary /healthz 503 during cold start can mean the API reached setup preflight before Postgres was ready; check the preflight JSON and wait for automatic DB retry before rotating secrets.',
