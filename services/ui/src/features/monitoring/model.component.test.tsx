...
describe('Monitoring model', () => {
  ...
  it('normalizes service and runner payloads', () => {
    ...
    expect(normalizeRunnerStatusValue('recently reconnected')).toBe('recovered');
    expect(normalizeRunnerSummary(null, [runner])).toMatchObject({ total: 1, online: 1, kubernetes: 1, capacity: 4 });
    expect(normalizeRunnerSummary({ total: '2', recovered: 1, unreachable: 1, queued_jobs: 3 }, [])).toMatchObject({ total: 2, recovered: 1, unreachable: 1, queuedJobs: 3 });
  });
