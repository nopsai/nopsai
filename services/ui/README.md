...
- `src/features/system/access` owns Access entity loading, user/service-account/role/policy/identity-provider catalogs, token workflows, destructive confirmations, resource catalogs, mutations, form state, summary/search presentation helpers, the shared basic-grant editor/reconciliation model, focused policy-rule rendering/normalization, sectioned drawer presentation, and role presentation. Basic grants keep the API value `root` but label that target as Global in the UI. The System Access shell keeps Basic and Advanced modes, top-row summary metrics, Pipeline Runs-style advanced section tabs, full-width table-first catalogs, full-height drawer-based create/edit flows, compact magnifier search, icon add actions, and sectioned editor review panels on the existing AAA/GitOps-compatible mutation paths.
- `src/features/system/dispatcher` owns Dispatcher status, polling, runner actions, the compact tabbed dispatcher overview, runtime-filtered table-first runner fleet with detail below the fleet after selection, runner detail links to advertised `runner:<runner-id>` system log sources, route edit/effective-routing tables, deployment scopes, install-template generation, private registry credential selection for runner installs, and guide state.
...
- `src/features/scopes/ScopeUsagePanel.tsx`, `scopes/ScopeWorkflowModals.tsx`, `triggers/TriggerRecentRuns.tsx`, `triggers/TriggerWorkflowModals.tsx`, `lab/LabRunControls.tsx`, `lab/LabDependencyPanel.tsx`, and `editor/ResourceWorkflowModals.tsx` own focused workflow rendering delegated by the remaining editor pages. `LabRunControls.tsx` also owns the right-rail run-readiness panel used above included dependencies.
...
- `src/features/lab/useLabSession.ts` and `useLabRunMutation.ts` own Lab session persistence, protected pipeline switching, persisted override compatibility, and authorized scoped-run submission.
...
- `src/features/editor/YamlEditorFullscreenDialog.tsx` owns expanded-editor top-bar validation for pipeline, step, trigger, and Lab YAML editors; `YamlValidationPanel.tsx` remains the shared inline validation presenter for non-fullscreen workflows.
