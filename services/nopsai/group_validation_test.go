...
func TestVisibleFolderGroupIDsIncludesAncestorContainers(t *testing.T) {
	teamID := 1
	devID := 2
	groups := []Group{
	...
	pathRecords := map[int]groupPathRecord{
		teamID: {ID: teamID, Path: "team-1"},
		devID:  {ID: devID, Path: "team-1/dev", ParentID: &teamID},
		3:      {ID: 3, Path: "other-team"},
	}
	allowedSet := map[string]struct{}{
		resourceKey(model.ResourceRef{Type: grantResourceFolder, ID: "team-1/dev"}): {},
	}

	visible, direct := visibleFolderGroupIDs(groups, pathRecords, allowedSet)
	...
	if _, ok := visible[3]; ok {
		t.Fatal("did not expect unrelated sibling folder to be visible")
	}
