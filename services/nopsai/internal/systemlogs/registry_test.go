...
func TestDefaultRegistryContainsOnlyPlatformServices(t *testing.T) {
	...
	for _, id := range []string{"nopsai", "aaa", "dispatcher", "git-bot", "ui", "docker-socket-proxy", "gotenberg", "db", "docker-runner", "k8s-runner"} {
		if _, ok := registry.Resolve(id); !ok {
			t.Fatalf("default registry missing %q", id)
		}
	}
	for _, id := range []string{"base", "agent", "pipeline"} {
		if _, ok := registry.Resolve(id); ok {
