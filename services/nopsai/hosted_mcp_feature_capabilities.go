	...
	return []hostedMCPFeatureCapability{
		...
		},
		{
			...
			Features: []string{
				...
				"Backend pipeline draft validation",
				"Step-level images",
				...
				"Backend reusable-step draft validation",
			...
			Notes: []string{
				"Backend validation endpoints return the shared {valid, errors, warnings} response shape for editor and automation drafts.",
		...
		},
		{
			...
			Features: []string{
				...
				"Pipeline final outputs",
				"Run creation",
				"Run listing",
				"Team/team run listing",
				"Root run listing",
				"Run details",
				"Run status",
				"Run approvals",
			...
			Tools: []string{
				...
				"nopsai.retry_pipeline_run_output",
			...
			APIRoutes: []string{
				...
				"POST /v1/runs/{runID}/outputs/{outputID}/retry",
			...
			Permissions: []hostedMCPPermission{
				...
				hostedMCPReadPermission("approval.approve", "team", "*"),
			...
			Notes: []string{
				...
				"Pipeline final outputs are configured in pipeline YAML and exposed as run-owned deliverables after finalization; pending or generating outputs can be cancelled without cancelling the pipeline run, and failed outputs can be retried individually; PDF/HTML use DocumentSpec and Excel uses SpreadsheetSpec structured source.",
				"Run creation, approval decisions, rerun, cancel, and delete require explicit confirmation before execution.",
			},
		},
		{
			Area: "Teams, applications, and delivery analysis",
			Features: []string{
				"Team and application inventory with paths used by every team-scoped resource",
				"Team-scoped run, spend, and reliability evidence",
				"Deterministic team delivery analysis with ranked findings and category scores",
				"Team inventory analysis for duplicate, near-identical, inherited, mixed-source, inactive, and deeply nested resources",
				"Deterministic pipeline analysis over runs, step timings, spend, and the stored definition",
				"Deterministic run analysis with a likely failure domain, first failure point, and what changed since the last success",
				"Recommended next tool call for the top finding",
			},
			Coverage: "first_class",
			Mode:     "read/analyze via REST, MCP, and the assistant; team writes stay on guarded REST routes",
			Tools: []string{
				"nopsai.list_teams",
				"nopsai.get_team",
				"nopsai.analyze_team",
			...
			APIRoutes: []string{
				...
				"GET /v1/runs/{runID}/approvals",
				"GET /v1/runs/{runID}/logs",
				"POST /v1/analysis/team",
				"POST /v1/analysis/pipeline",
				"POST /v1/analysis/run",
			},
			Permissions: []hostedMCPPermission{
				hostedMCPReadPermission("team.list", "team", "*"),
				hostedMCPReadPermission("team.read", "team", "*"),
				hostedMCPReadPermission("pipeline.read", "pipeline", "*"),
				hostedMCPReadPermission("pipeline_run.read", "pipeline_run", "*"),
			},
			Notes: []string{
				"Analysis findings, severities, and scores are computed deterministically from permission-filtered monitoring evidence; the LLM explains them and never derives the score.",
				"The health score uses the published formula: baseline 100 minus critical x 25, high x 15, medium x 8, low x 3, opportunity x 1, clamped to 0-100. It matches the UI analysis engine.",
				"A monitoring source that cannot be read is reported as a limitation and lowers ok to false; an unreadable category never scores as healthy.",
				"Analysis is read-only and applies nothing; next_actions name the tool call that investigates the top finding.",
			},
		},
		{
			Area: "Team dashboards",
			Features: []string{
				"Team-owned dashboards",
				"Multiple dashboards per team",
				"Dashboard sections",
				"Dashboard final output type",
				"DashboardSpec structured content validation",
				...
				"Older-run replacement protection",
				"Full dashboard, section, and source refresh orchestration",
				"Strict and best-effort refresh modes",
				"Per-source refresh progress, cancellation, retry failed, timeout, concurrency, and idempotency",
				"Scheduled dashboard refreshes with cron, timezone, service-account execution, and run-now",
				"GitOps dashboards and templates with validation, drift, source provenance, and managed-state pruning",
			},
			Coverage: "first_class",
			Mode:     "read/manage/refresh/schedule via REST and UI; assistant reads and can start confirmed refreshes and schedule runs; pipeline publication through validated final outputs",
			Tools: []string{
				"nopsai.call_api",
			...
			Permissions: []hostedMCPPermission{
				hostedMCPReadPermission("dashboard.list", "dashboard", "*"),
				hostedMCPReadPermission("dashboard.read", "dashboard", "*"),
				hostedMCPReadPermission("dashboard.create", "dashboard", "*"),
				hostedMCPReadPermission("dashboard.update", "dashboard", "*"),
				hostedMCPReadPermission("dashboard.delete", "dashboard", "*"),
				hostedMCPReadPermission("dashboard.publish", "dashboard", "*"),
				hostedMCPReadPermission("dashboard.refresh", "dashboard", "*"),
				hostedMCPReadPermission("dashboard.manage_sources", "dashboard", "*"),
				hostedMCPReadPermission("pipeline_run.read", "pipeline_run", "*"),
			},
			Notes: []string{
		...
		},
		{
			...
			Tools: []string{
				...
				"nopsai.list_knowledge_connections",
				"nopsai.get_pipeline_knowledge_context",
			...
			APIRoutes: []string{
				...
				"PUT /v1/knowledge-connections/{connectionID...}",
				"PATCH /v1/knowledge-connections/{connectionID...}",
				"DELETE /v1/knowledge-connections/{connectionID...}",
				"POST /v1/knowledge-connections/{connectionID}/test",
				"GET /v1/knowledge-connections/{connectionID}/pages",
				"POST /v1/knowledge-connections/{connectionID}/resolve-page",
				"GET /v1/knowledge-context-connections",
				"POST /v1/knowledge-context-connections",
				"GET /v1/knowledge-context-connections/{connectionID...}",
				"PUT /v1/knowledge-context-connections/{connectionID...}",
				"PATCH /v1/knowledge-context-connections/{connectionID...}",
				"DELETE /v1/knowledge-context-connections/{connectionID...}",
				"POST /v1/knowledge-context-connections/{connectionID}/test",
				"GET /v1/knowledge-context-connections/{connectionID}/pages/search",
				"POST /v1/knowledge-context-connections/{connectionID}/resolve-page",
			},
			Permissions: []hostedMCPPermission{
				...
				hostedMCPReadPermission("knowledge_connection.read", "knowledge_connection", "*"),
			},
			Notes: []string{
		...
		},
		{
			...
			Features: []string{
				...
				"Trigger manifests",
				"Backend trigger override draft validation",
				"Event matching",
				"Branch globs",
				"Tag globs",
				"Skipped branches",
				...
				"External trigger run team routing",
				"Branch listing",
			},
			Coverage: "partial",
		...
		},
		{
			...
			Features: []string{
				...
				"Config repository draft-bundle dry-run validation",
			...
			Tools: []string{
				...
				"nopsai.cancel_system_config_sync",
				...
				"nopsai.cancel_all_config_repos_sync",
				"nopsai.get_system_status",
			},
			Resources: []string{
				"nopsai://system/status",
			...
			APIRoutes: []string{
				...
				"POST /v1/system/config/sync/cancel",
				...
				"POST /v1/teams/{teamID}/config-repository/validate",
			},
			Permissions: []hostedMCPPermission{
				...
				hostedMCPReadPermission("config_repo.read", "team", "*"),
				hostedMCPReadPermission("config_repo.sync", "team", "*"),
				hostedMCPReadPermission("config_repo.manage", "team", "*"),
			},
			Notes: []string{
				"Config repo validate endpoints reuse the config-sync parser in dry-run mode and perform no writes.",
				"Config repo sync, cancel, and write tools use the existing GitOps API workflow and require explicit confirmation for mutating operations.",
			},
		},
		{
			Area: "Notifications",
			...
			APIRoutes: []string{
				...
				"DELETE /v1/teams/{teamID}/notifications",
			},
			Permissions: []hostedMCPPermission{
		...
		},
		{
			...
			Features: []string{
				...
				"Schedule AI usage comparisons",
				"Pipeline health explanations",
				"Optimization opportunity discovery",
			},
			Coverage: "first_class",
			Mode:     "analytics, comparisons, health explanations, cost/statistics/dispatcher, saved views, alerts, and recommendation operations first-class",
			Tools: []string{
				"nopsai.call_api",
			...
			Notes: []string{
				...
				"Dedicated analytics tools and comparison/explanation aliases accept the shared monitoring filter set.",
			},
		},
		{
			...
			Features: []string{
				...
				"Live system logs",
				"Local login",
				"Mandatory break-glass administrator",
				"OIDC identity providers",
				"GitHub OAuth2 identity provider",
				"One active external identity provider",
				"Access token flow",
				"Refresh token flow",
				...
				"MCP registry sync",
				"Credential sync",
				"GitHub App manifest registration",
			...
			APIRoutes: []string{
				...
				"GET /v1/git-apps/github",
				"PUT /v1/git-apps/github",
				"GET /v1/git-apps/github/installations",
				"POST /v1/git-apps/github/installations",
				"GET /v1/git-apps/github/installations/{installationID}",
				"DELETE /v1/git-apps/github/installations/{installationID}",
				...
				"DELETE /v1/system/dispatcher/runners/{runnerID}",
			...
			Permissions: []hostedMCPPermission{
				hostedMCPReadPermission("system.read", "system", "config"),
				hostedMCPReadPermission("system.update", "system", "config"),
				hostedMCPReadPermission("system.read", "system", "models"),
				hostedMCPReadPermission("system.update", "system", "models"),
				hostedMCPReadPermission("system.read", "system", "agent-roles"),
				hostedMCPReadPermission("system.update", "system", "agent-roles"),
			...
			Notes: []string{
				...
				"GitHub App registration and installation are browser redirect flows: an assistant can read and edit Git Apps records, but creating or installing an App has to happen in an operator session on GitHub.",
			},
		},
		{
			...
			Features: []string{
				...
				"Product roles",
				"Hybrid local and IdP authorization",
				"Provider-owned grant metadata",
				"Access-grant management",
				"Per-resource access controls",
				"Runtime use checks",
				"Resource visibility modes",
				"Team-path inheritance",
			...
			APIRoutes: []string{
				...
				"PUT /v1/admin/identity-providers",
				"PUT /v1/admin/identity-providers/{provider}",
				"DELETE /v1/admin/identity-providers/{provider}",
			},
			Permissions: []hostedMCPPermission{
			...
			Notes: []string{
				...
				"Identity-provider sync owns only rows marked source=idp for that provider; local UI/API/GitOps grants stay editable and are included in effective access.",
			},
		},
		{
