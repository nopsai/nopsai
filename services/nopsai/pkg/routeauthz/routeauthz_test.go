...
func TestMutatingCatalogRoutesRequireAuthzOrExplicitHandlerAuthorization(t *testing.T) {
	deferrals := map[string]string{
		...
		"POST /v1/analysis/pipeline":                                               "Subject analysis is read-only and authorizes every evidence source as the caller through the API bridge.",
		"POST /v1/analysis/run":                                                    "Subject analysis is read-only and authorizes every evidence source as the caller through the API bridge.",
		"POST /v1/analysis/team":                                                   "Subject analysis is read-only and authorizes every evidence source as the caller through the API bridge.",
...
func TestMapRequestUsesUpdatedLowLevelActions(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "system config sync cancel uses system config sync update",
			method:     http.MethodPost,
		...
		{
			name:       "global config repo sync cancel uses system resource",
			method:     http.MethodPost,
		...
		{
			name:       "all config repo sync cancel uses system resource",
			method:     http.MethodPost,
		...
		},
		{
			...
			method:     http.MethodDelete,
			path:       "/v1/system/agent-roles/sre",
			wantAction: "system.read",
			wantType:   "system",
			wantID:     "agent-roles",
			wantFilter: true,
		...
		{
			name:       "team config repository sync cancel uses team path",
			method:     http.MethodPost,
			path:       "/v1/teams/team-1/config-repository/sync/cancel",
			wantAction: "config_repo.sync",
			wantType:   "team",
			wantID:     "team-1",
		},
		...
		},
		{
			...
			method:     http.MethodPut,
			path:       "/v1/teams/team-1/models/standard",
		...
		},
		{
			...
			method:     http.MethodPut,
			path:       "/v1/teams/team-1%2Fplatform/models/default",
			wantAction: "team.update",
			wantType:   "team",
			wantID:     "team-1/platform",
		},
		{
			...
			method:     http.MethodPut,
			path:       "/v1/teams/team-1%2Fplatform/agent-roles/default",
			wantAction: "team.update",
			wantType:   "team",
			wantID:     "team-1/platform",
		},
