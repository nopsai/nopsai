...
func TestNopsAIGitOpsPlatformReleasePipelineValidates(t *testing.T) {
	...
	requireContains(t, steps["publish-base-image"].GetScript(), "--tag \"$REGISTRY/nopsai-base:latest\"")
	requireContains(t, steps["publish-base-image"].GetScript(), "--tag \"$REGISTRY/$image_name:latest\"")
