...
func hostedMCPFinalTools() []hostedMCPTool {
	...
	return []hostedMCPTool{
		...
		toolDef("nopsai.generate_runner_compose", "Generate Docker Compose runner installation artifacts.", "system.update", "dispatcher", "runners", objectSchema(map[string]any{"runner_id": stringSchema(), "runner_uid": stringSchema(), "runner_scopes": stringSchema(), "runner_capacity": numberSchema(), "query": objectSchema(map[string]any{})})),
		toolDef("nopsai.generate_kubernetes_runner_manifest", "Generate Kubernetes runner installation manifest.", "system.update", "dispatcher", "runners", objectSchema(map[string]any{"runner_id": stringSchema(), "runner_uid": stringSchema(), "runner_scopes": stringSchema(), "runner_capacity": numberSchema(), "namespace": stringSchema(), "service_account": stringSchema(), "storage_class": stringSchema(), "query": objectSchema(map[string]any{})})),
		toolDef("nopsai.generate_runner_bootstrap_command", "Generate a runner bootstrap command. Sensitive response requires include_sensitive_response:true.", "system.update", "dispatcher", "runners", objectSchema(map[string]any{"runner_id": stringSchema(), "runner_uid": stringSchema(), "runner_scopes": stringSchema(), "runner_capacity": numberSchema(), "registry_credential_ref": stringSchema(), "registry_credential_refs": arraySchema(stringSchema()), "include_sensitive_response": booleanSchema(), "query": objectSchema(map[string]any{})})),
		toolDef("nopsai.generate_kubernetes_runner_bootstrap_command", "Generate a Kubernetes runner bootstrap command. Sensitive response requires include_sensitive_response:true.", "system.update", "dispatcher", "runners", objectSchema(map[string]any{"runner_id": stringSchema(), "runner_uid": stringSchema(), "runner_scopes": stringSchema(), "runner_capacity": numberSchema(), "namespace": stringSchema(), "service_account": stringSchema(), "storage_class": stringSchema(), "registry_credential_ref": stringSchema(), "registry_credential_refs": arraySchema(stringSchema()), "include_sensitive_response": booleanSchema(), "query": objectSchema(map[string]any{})})),
		...
		toolDef("nopsai.eject_runner", "Revoke a runner ID and remove its dispatcher registration. Requires confirm:true.", "system.update", "dispatcher", "runners", objectSchema(map[string]any{"runner_id": stringSchema(), "confirm": booleanSchema()})),
		...
		toolDef("nopsai.get_effective_permissions", "Check the current subject's effective permission for an action and resource.", "iam.admin", "iam", "admin", objectSchema(map[string]any{"action": stringSchema(), "resource_type": stringSchema(), "resource_id": stringSchema(), "team_path": stringSchema(), "run_team_path": stringSchema()})),
...
func (a *App) executeHostedMCPFinalTool(ctx context.Context, subject aaamodel.Subject, name string, args map[string]any) (map[string]any, bool, error) {
	...
	case "nopsai.eject_runner":
		return a.hostedMCPFinalAPITool(ctx, subject, http.MethodDelete, "/v1/system/dispatcher/runners/"+hostedMCPPathTail(stringArg(args, "runner_id")), nil, boolArg(args, "confirm", false), false, true, "Removing a runner revokes its ID, disconnects live streams, and may requeue in-flight work."), true, nil
