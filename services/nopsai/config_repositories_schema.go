...
var configRepositorySchemaStatements = []string{
	`CREATE TABLE IF NOT EXISTS config_repositories (
		...
		provider TEXT NOT NULL DEFAULT 'github' CHECK (provider IN ('github', 'gitlab', 'bitbucket', 'gitea')),
	...
	`ALTER TABLE config_repositories DROP CONSTRAINT IF EXISTS config_repositories_provider_check`,
	`ALTER TABLE config_repositories ADD CONSTRAINT config_repositories_provider_check CHECK (provider IN ('github', 'gitlab', 'bitbucket', 'gitea'))`,
	`ALTER TABLE config_repositories ADD COLUMN IF NOT EXISTS credential_ref TEXT NOT NULL DEFAULT ''`,
