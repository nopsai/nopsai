...
func TestAssistantUsageForAssistantReplyEstimatesWhenLLMUsageMissing(t *testing.T) {
	...
	if usage.TotalTokens == 0 || usage.CompletionTokens == 0 || !usage.Estimated {
		t.Fatalf("usage = %#v, want estimated visible reply tokens", usage)
	}
...
func TestAssistantUsageForUserMessageTracksVisibleContentOnly(t *testing.T) {
	...
	if usage.ContentTokens == 0 || usage.TotalTokens == 0 || !usage.Estimated {
		t.Fatalf("usage = %#v, want estimated visible user content tokens", usage)
	}
	if usage.PromptTokens != 0 || usage.CompletionTokens != 0 || usage.LLMCalls != 0 {
...
func TestAssistantConversationUsageFromMessagesRollsUpMessageMetrics(t *testing.T) {
	messages := []assistantMessage{
		{Usage: assistantMessageUsage{ContentTokens: 4, TotalTokens: 4, Estimated: true}},
	...
	if usage.MessageCount != 2 || usage.ContentTokens != 7 || usage.PromptTokens != 10 || usage.CompletionTokens != 5 || usage.TotalTokens != 19 {
