...
func (a *App) buildAssistantPlannerPrompt(ctx context.Context, subject aaamodel.Subject, conversation assistantConversation, content string, plan assistantTurnPlan, toolCalls []assistantToolActivity, remainingToolCalls int, iteration int) string {
	payload := map[string]any{
		...
		"available_tools":      assistantPlannerToolCatalog(a.hostedMCPToolsForSubject(ctx, subject)),
...
Create a safe hosted MCP tool plan from the user's request and observed tool results.
Use only tool names from available_tools. Never invent tools. Never request direct database access.
