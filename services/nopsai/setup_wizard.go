...
func setupReadme(profile string) string {
	return fmt.Sprintf("# NopsAI starter config\n\nWorkspace: `%s`\n\nThis repository contains starter resources for the first NopsAI workspace bootstrap. Keep secret values outside this repository and reference them by secret name from LLM, MCP, and pipeline configuration.\n", setupAccessFolder(profile))
}
