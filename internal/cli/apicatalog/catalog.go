...
func queryParameters(path string) []QueryParameter {
	...
	case "/v1/auth/oauth2/{provider}/start":
		add("return_to", "Relative UI path to return to after login.", "/")
	case "/v1/auth/oauth2/{provider}/callback":
		addRequired("code", "OAuth2 authorization code returned by the provider.", "provider-code")
		addRequired("state", "Opaque OAuth2 state generated by the start endpoint.", "state")
