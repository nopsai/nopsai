...
func TestKubernetesValuesInstallPlanRendersEditableValues(t *testing.T) {
	...
	for _, required := range []string{
		...
		"`database-url` and `postgres-password` must contain the same database password",
		"changing the Secret alone does not rotate the existing database role password",
		"| Bootstrap admin password | `initial-admin-password` |",
		"## Registry Pull Secret",
		"kubectl -n nopsai-system create secret docker-registry nopsai-registry",
		"imagePullSecrets:",
		"nopsai-api",
		"nopsai-runner",
		...
		"DATABASE_URL=\"postgres://nopsai_user:${POSTGRES_PASSWORD}@postgres:5432/nopsai_db?sslmode=disable\"",
		"--from-literal=postgres-password=\"$POSTGRES_PASSWORD\"",
		"kubectl -n nopsai-system create secret generic nopsai-prod-secrets",
	...
	if lock.FileHashes["README.md"] == "" || lock.FileHashes["prod/values.yaml"] == "" {
		t.Fatalf("lock does not hash generated notes and values: %#v", lock.FileHashes)
	}
